...
const LoginPage = () => {
  ...
  return (
    <section className='flex flex-col justify0center mt-20 max-w-[400px] m-auto'>
      <div className='p-6 bg-white rounded-md shadow-md'>
        ...
        <form className='mt-6' onSubmit={handleSubmit(onSubmit)}>
          <div className='mb-2'>
            ...
            <input type='email' id='email' className='w-full px-4 py-2 mt-2 bg-white border rounded-md' {...register('email', userEmail)} />
            {
              errors?.email && 
              <div>
          ...
          <div className='mt06'>
            <button type='submit' className='w-full bg-black text-white px-4 py-2 rounded-md hover:bg-gray-700 duration-200'>로그인</button>
          </div>

          <p className='mt-8 text-xs font-light text-center text-gray-700'>
            아이디가 없다면?{' '}
            <a href='/register' className='font-medium hover:underline'>회원가입</a>
            </p>
        </form>
      </div>
    </section>
